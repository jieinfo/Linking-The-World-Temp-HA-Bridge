...
## 1.4.0 - Configuration identity, cleanup, and quality gates

- 配置向导会先规范化主机地址，再按地址与端口检查已有配置；相同域名的大小写、末尾点号和
  等价 IPv6 写法会在建立 TCP 连接前直接识别为重复配置，避免无意义地再次登录主机。
  现有配置项、实体 unique ID 和设备身份保持不变；不同域名或 IP 是否指向同一主机仍不作
  未经协议证明的推断。
- 永久删除 Home Assistant 配置项时同步删除该配置项专属的面板发现历史，并继续清理关联
  Repairs；普通卸载、重载、重启和版本升级仍会保留面板数据与实体身份。
- 增加模式切换过程中卸载的并发回归测试，确保连接中断后不会继续发送模式或恢复开机命令；
  增加 1,000 组确定性畸形状态字段测试，防止异常输入逃逸到会话主循环。
- CI 新增 Ruff 与 mypy 静态检查，并将原生集成测试拆为最低支持环境和最新稳定环境两条轨道；
  同时修正静态检查发现的可空状态收窄、缺失类型导入和无用测试代码，不改变协议与实体行为。

## 1.3.4 - Bounded runtime delivery and malformed-panel isolation

- 将握手与登录响应队列限制为只接收协议类型 `1` 和 `2` 的会话响应；登录后的状态推送继续
  通过回调实时处理，不再同时永久堆积在无人消费的队列中，避免长期运行时内存持续增长。
- 在面板状态协议入口要求设备 MAC 必须严格为 8 字节；长度异常的面板报文只作为无效状态
  忽略并计入诊断，不再进入面板登记流程、抛出异常或导致整条 MC7021 会话重连。
- 增加 3,000 帧持续推送的有界队列回归测试，以及异常 MAC 后连接存活并继续处理合法状态的
  实机路径测试；正常总控、温控面板状态和控制协议保持不变。
