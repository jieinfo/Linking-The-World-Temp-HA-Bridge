...
## 1.2.8 - Controller-native winter humidifier behavior

- 保留官方“冬季加湿”名称与原实体 unique ID，解除仅制热模式可操作的 HA 侧限制；只要
  主机连接和控制权限正常，开关在制冷、制热、通风和除湿模式下均可发送命令。
- 非制热模式下开启后，MC7021 若立即回报冬季加湿仍为关闭，集成将其识别为主机对当前
  模式操作的明确拒绝，及时结束命令等待并保持开关关闭；“最近控制命令状态”会显示
  “主机未接受当前操作”，不再误触发确认超时或 Repair。
- 保留制热模式下原有 ON/OFF 主动上报确认、串行命令队列和只读模式保护，并增加非制热
  模式实体可用性、命令发送、主机拒绝及队列行为回归测试。
